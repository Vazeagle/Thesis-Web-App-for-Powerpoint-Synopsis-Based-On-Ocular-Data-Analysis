...
class TextBox(PowerPointObject):
    def __init__(self, xAxisStart, yAxisStart, width, height, ObjectCategory, slideNumber, objCounter, text, linesWidth, lineSpacer, yStep, fontName, fontSize, marginLeft, marginRight, marginTop, marginBottom, scaleMultiplier, groupID):
    ...
    def getValues(self):
        ...
        print("Scale Multiplier: ", self.scaleMultiplier)
        print("GroupID: ", self.groupID)
        print("\n")
        #return these values as dict
        return {"TextBoxXstart(removedMargin)":self.xAxisStart, "TextBoxYstart(removedMargin)":self.yAxisStart, "TextBoxWidth(removedMargin)": self.width, "TextBoxHeight(removedMargin)":self.height,"ObjectCategory":self.ObjectCategory, "slideNumber":self.slideNumber, "objCounter":self.objCounter, "ParText":self.text, "EachLineWidth":self.linesWidth, "LineSpacer":self.lineSpacer, "LineSizeY":self.yStep, "FontName":self.fontName, "FontSize":self.fontSize, "MarginLeft":self.marginLeft, "MarginRight":self.marginRight, "MarginTop":  self.marginTop, "MarginBot": self.marginBottom, "ScaleMultiplier":self.scaleMultiplier, "GroupID":self.groupID}

class Image(PowerPointObject):
    def __init__(self, xAxisStart, yAxisStart, width, height, ObjectCategory, slideNumber, objCounter, imageLoc, imageExtensionType, scaleMultiplier, groupID):
        super().__init__(xAxisStart, yAxisStart, width, height, ObjectCategory, slideNumber, objCounter)
    ...
    def getValues(self):
        ...
        print("GroupID: ", self.groupID)
        print("\n")
        return {"PictureXstart":self.xAxisStart, "PictureYstart":self.yAxisStart, "PictureWidth": self.width, "PictureHeight":self.height,"ObjectCategory":self.ObjectCategory, "slideNumber":self.slideNumber, "objCounter":self.objCounter, "imageLoc":self.imageLoc, "PictureExtensionType":self.imageExtensionType, "ScaleMultiplier":self.scaleMultiplier, "GroupID":self.groupID}
